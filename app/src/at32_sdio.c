...
static sdio_error_t sd_bus_wide_enable (confirm_state new_state);
static sdio_error_t mmc_switch (uint8_t set, uint8_t index, uint8_t value);
static sdio_error_t sd_switch (uint32_t mode, uint32_t group, uint8_t value,
                        uint8_t *rsp);
static sdio_error_t check_card_programming (uint8_t *p_status);
static sdio_error_t speed_change (uint8_t speed);
static sdio_error_t scr_find (void);
...
sdio_info_t sd_card_info;                     /* sd card information */



static uint8_t sdio_data_buffer[512] ALIGNED (4);
 ...
 * @retval sdio_error_t: sd card error code.
 */
sdio_error_t sd_card_init (void)
{
   sdio_error_t status = SD_OK;
 ...
 * @param  card_info: pointer to a sdio_info_t structure that contains all sd
 * card information.
 * @retval sdio_error_t: sd card error code.
 */
sdio_error_t sd_card_info_get (sdio_info_t *card_info)
{
   sdio_error_t status  = SD_OK;
   uint8_t tmp          = 0;
       ...
       (card_type == SDIO_MULTIMEDIA_CARD))
   {
      ...
      card_info->card_blk_size = 1
                                 << (card_info->sd_csd_reg.max_read_blk_length);
 ...
 * @retval sdio_error_t: sd card error code.
 */
sdio_error_t sd_wide_bus_operation_config (sdio_bus_width_type mode)
{
   sdio_error_t status = SD_OK;
 ...
 * @retval sdio_error_t: sd card error code.
 */
sdio_error_t sd_device_mode_set (uint32_t mode)
{
   sdio_error_t status = SD_OK;
 ...
 * @retval sdio_error_t: sd card error code.
 */
sdio_error_t sd_deselect_select (uint32_t addr)
{
 ...
 * @retval sdio_error_t: sd card error code.
 */
sdio_error_t sdio_command_data_send (sdio_command_struct_type *sdio_cmd_init_t,
 ...
 * @retval sdio_error_t: sd card error code.
 */
sdio_error_t sd_blocks_erase (long long addr, uint32_t nblks)
{
   sdio_error_t status = SD_OK;
 ...
 * @retval sdio_error_t: sd card error code.
 */
sdio_error_t sd_block_read (uint8_t *buf, long long addr, uint16_t blk_size)
{
   sdio_error_t status = SD_OK;
   uint32_t response   = 0;
   uint8_t power;
 ...
 * @retval sdio_error_t: sd card error code.
 */
sdio_error_t sd_mult_blocks_read (uint8_t *buf, long long addr,
 ...
 * @retval sdio_error_t: sd card error code.
 */
sdio_error_t sd_mult_blocks_write (const uint8_t *buf, long long addr,
                                   uint16_t blk_size, uint32_t nblks)
{
   sdio_error_t status = SD_OK;
   uint8_t power = 0, card_state = 0;
 ...
 * @retval sdio_error_t: sd card error code.
 */
sdio_error_t sd_irq_service (void)
{
 ...
 * @retval sdio_error_t: sd card error code.
 */
sdio_error_t command_rsp6_error (uint8_t cmd, uint16_t *prca)
{
   sdio_error_t status = SD_OK;
 ...
 * @retval sdio_error_t: sd card error code.
 */
sdio_error_t sd_status_send (uint32_t *p_card_status)
{
   sdio_error_t status = SD_OK;
